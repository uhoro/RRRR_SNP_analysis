...
OPTIONS:
    -t, --txt_path TXTPATH    Specify the path to text file listing split_axt file names (required)
    -d, --dir_path DIRPATH    Specify the path to split_axt file directory (required)
    -o, --kaks_out KaKsOUT    Specify the path to kaks file directory 
    -h, --help                 Display this help message and exit

EXAMPLES:
    ${SCRIPT_NAME} -t /pasteur/zeus/projets/p02/ecb_bioinfo/uhoro/RRRR_KaKs/test_data_pipeline/C1G11_C1C5v2_split_axt_list.txt -d /pasteur/zeus/projets/p02/ecb_bioinfo/uhoro/RRRR_KaKs/test_data_pipeline/C1G11_C1C5v2 -o /pasteur/zeus/projets/p02/ecb_bioinfo/uhoro/RRRR_KaKs/test_KaKs_pipeline/C1G11_C1C5v2
...
TEMP=$(getopt -o t:d:o:h --long text_path:,dir_path:,kaks_out:,help \
              -n "$SCRIPT_NAME" -- "$@")
...
if [ -z "${KaKsOUT}" ]; then
    echo "Error: KaKs output path is required. Use -o or --kaks_out to specify." >&2
    echo "Use ${SCRIPT_NAME} -h for help." >&2
    exit 1
fi
...
echo "  kaks output directory path: ${KaKsOUT}"
echo ""
echo "=================================="
...
RESULT_FULL_PATH="${KaKsOUT}/${FILE_TO_PROCESS}.kaks"
...
KaKs -i ${FILE_FULL_PATH} -o ${RESULT_FULL_PATH} -m MYN
